...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="group w-full bg-white border border-[#e8d5c4] rounded-md
      overflow-hidden hover:-translate-y-1.5
      hover:shadow-[0_16px_40px_rgba(139,94,82,0.12)]
      ...
      <div className="relative h-[200px] sm:h-[220px] md:h-[240px] overflow-hidden"
        ...
        <div className="w-full h-full group-hover:scale-105 transition-transform duration-500
          ...
          <span className="text-[80px] opacity-20 select-none">✿</span>
        </div>
      ...
      <div className="px-4 pt-3.5 pb-4">
        ...
        <h3 className="font-[Cormorant_Garamond] text-[18px] md:text-[20px] font-normal
          text-[#5c3d35] leading-tight mb-1">{name}</h3>
        <p className="text-[12px] font-light text-[#a08878] leading-relaxed mb-3 hidden sm:block">
          {desc}
        </p>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-baseline gap-1.5">
            <span className="font-[Cormorant_Garamond] text-[20px] md:text-[22px]
              font-medium text-[#5c3d35]">₹{price}</span>
            {oldPrice && (
              <span className="text-[12px] font-light text-[#b09088] line-through">₹{oldPrice}</span>
            )}
          </div>
          <button onClick={handleAdd}
