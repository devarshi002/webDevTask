...
export default function CartDrawer({ isOpen, onClose }) {
  const { cartItems, increaseQty, decreaseQty, removeFromCart, totalItems, totalPrice } = useCart();
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className={`fixed top-0 right-0 h-full w-full sm:w-[400px] bg-[#fdf6f0] z-50 shadow-2xl
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#e8d5c4]">
          <div className="flex items-center gap-2">
            <ShoppingBag size={18} strokeWidth={1.5} className="text-[#8b5e52]" />
            <span className="font-[Cormorant_Garamond] text-xl text-[#5c3d35]">Your Basket</span>
            {totalItems > 0 && (
              <span className="bg-[#c4957a] text-white text-[10px] w-5 h-5 rounded-full
                flex items-center justify-center">{totalItems}</span>
            )}
          </div>
          <button onClick={onClose} className="text-[#8b5e52] hover:text-[#c4957a] transition-colors">
        ...
        {cartItems.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-8">
            ...
            <p className="font-[Cormorant_Garamond] text-2xl text-[#8b5e52]">Your basket is empty</p>
          ...
          <>
              ...
              {cartItems.map(item => (
                <div key={item.id} className="flex gap-4 bg-white border border-[#e8d5c4] rounded-md p-3">
                  ...
                  <div className="flex flex-col items-end justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-2 border border-[#e8d5c4] rounded-sm px-2 py-1">
            ...
            <div className="px-6 py-5 border-t border-[#e8d5c4] bg-white">
              ...
              <div className="flex items-center justify-between mb-4">
                <span className="text-[13px] font-light text-[#7a5c52] tracking-wide">Subtotal</span>
                <span className="font-[Cormorant_Garamond] text-xl text-[#5c3d35]">₹{totalPrice}</span>
              </div>
              <button
